-- 032_fix_ashram_spelling.sql
-- Fix typo 'planing' to 'planning' in ashram column constraints and existing data
...
WHERE ashram = 'Staying Single (Not planing to Marry)';
...
WHERE (hierarchy->>'ashram') = 'Staying Single (Not planing to Marry)';
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'profile_update_requests') THEN
        ...
        WHERE (requested_changes->>'ashram') = 'Staying Single (Not planing to Marry)';

        UPDATE profile_update_requests
        SET current_values = current_values || jsonb_build_object('ashram', 'Staying Single (Not planning to Marry)')
        WHERE (current_values->>'ashram') = 'Staying Single (Not planing to Marry)';
    END IF;
END $$;
