...
export async function findOrCreateFolder(accessToken: string, folderName: string, parentFolderId: string): Promise<string> {
    ...
    while (retryCount < maxRetries) {
        ...
        // Sort by createdTime ascending to easily pick the oldest
        ...
        if (searchRes.ok) {
            ...
            if (searchData.files && searchData.files.length > 0) {
                // Return the FIRST one (oldest, because of orderBy=createdTime asc)
        ...
        if (createRes.ok) {
            ...
            // 3. POST-CREATION VALIDATION (Crucial for Race Conditions)
            // Immediately search again to see if we created a duplicate or if someone else did
            // We want to converge everyone to the OLDEST folder ID.
            ...
            if (validationRes.ok) {
                ...
                if (validationData.files && validationData.files.length > 1) {
                    ...
                    // The API returns them sorted by createdTime ASC, so index 0 is the oldest.
                    ...
                    if (oldestFolder && oldestFolder.id !== newFolderId) {
                        ...
                        // Per user request: DO NOT DELETE/TRASH the new folder. Just abandon it and use the old one.
                        // This leaves a potential empty duplicate folder, but guarantees no data loss.
        ...
        // If folder already exists (409) or other errors, retry logic
        if (createRes.status === 409 || errorData.error?.message?.toLowerCase().includes('duplicate')) {
            ...
            retryCount++
            await new Promise(resolve => setTimeout(resolve, 300 * retryCount)) // Backoff
            continue
        }
...
export async function initializeUserDriveFolders(userName: string) {
    ...
    try {
        ...
        if (!mainFolderId) {
            throw new Error('MAIN_DRIVE_FOLDER_ID not set');
        }
        ...
        // Run in parallel for speed, but individual functions handle their own race conditions safely
        ...
        // We throw so the caller knows it failed, though arguably we might not want to fail the whole approval 
        // if just folder creation fails. But for now, let's treat it as part of the process.
        throw error;
    }
}
