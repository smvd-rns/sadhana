...
import {
    ...
    Search, AlertCircle, Eye, Building2
} from 'lucide-react';
import {
...
export default function MDSadhanaReportPage() {
    ...
    const [selectedCenter, setSelectedCenter] = useState<string>(''); // empty means All Centers
    ...
    const ROLE_NAMES = ['managing_director', 'super_admin', 'director', 'central_voice_manager', 'youth_preacher'];
    const isMD = userData && (Array.isArray(userData?.role) ? userData.role : [userData?.role]).some(r =>
        ADMIN_ROLES.includes(Number(r)) || ROLE_NAMES.includes(String(r))
    );

    // Initial Context Fetch (Temples)
    useEffect(() => {
        if (!userData) return;
        if (!isMD) {
            ...
            try {
                ...
                if (data && data.length > 0) {
                    ...
                    if (profileTempleName) {
                        ...
                        toast.error('No temple assigned to your profile.');
                    }
                }
            } catch (err) {
        ...
        fetchAssignments();
    }, [userData, isMD, router]);

    // Fetch Centers when Temple changes
    useEffect(() => {
        ...
        const loadCenters = async () => {
            ...
            try {
                const { data, error } = await supabase
                    .from('centers')
                    .select('id, name')
                    .eq('temple_name', selectedTemple.name)
                    .order('name');

                if (error) throw error;
                setCenters(data || []);
                setSelectedCenter(''); // Reset to all centers
            } catch (err) {
    ...
    // Fetch Users when Center or Temple changes
    useEffect(() => {
        if (!selectedTemple?.name) return;
        ...
        const loadUsers = async () => {
            ...
            try {
                ...
                const filtered = (data || []).filter((u: any) => {
                    ...
                    if (uTemple !== selectedTemple.name) return false;

                    if (selectedCenter !== '') {
                        const uCenter = u.current_center || uH?.currentCenter?.name || uH?.center || u.center || '';
                        return uCenter === selectedCenter;
                    }
        ...
        loadUsers();
    }, [selectedTemple, selectedCenter]);


    const handleFetchReports = async () => {
        if (users.length === 0) {
            toast.error('No devotees found for current selection.');
            return;
        }
    ...
    // Auto-fetch when users array is populated initially
    useEffect(() => {
    ...
    const aggregatedData = useMemo(() => {
        ...
        const result = Object.values(dataByUserId).map(uData => {
            ...
            return {
                id: uData.user.id,
                name: uData.user.name.split(' ')[0],
                fullName: uData.user.name,
                email: uData.user.email,
                center: uData.user.hierarchy?.currentCenter || uData.user.hierarchy?.center || uData.user.current_center || 'Unknown',
                user: uData.user,
    ...
    return (
        <div className="p-3 sm:p-6 max-w-7xl mx-auto space-y-4 sm:space-y-6 animate-in fade-in duration-500">
            {/* Header */}
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-4 sm:p-5 rounded-2xl border border-gray-100 shadow-sm">
                <div>
                    <div className="flex items-center gap-2 text-gray-400 mb-2">
                        <Link href="/dashboard/managing-director" className="hover:text-amber-600 transition-colors flex items-center">
                    ...
                    <p className="text-sm text-gray-500 font-medium mt-1">Temple-wide overview of spiritual progress</p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row w-full lg:w-auto gap-3">
                    {assignedTemples.length > 0 && (
                        <div className="w-full sm:min-w-[200px]">
                            <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1">
                                <Building2 className="h-3 w-3" /> Temple
                            </label>
                            <select
                                ...
                                className="w-full bg-gray-50 border border-gray-200 text-gray-800 text-sm font-bold py-1.5 px-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all appearance-none"
                            >
                    ...
                    <div className="w-full sm:min-w-[200px]">
                        <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Center Division</label>
                        <select
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-white border-b border-gray-100">
                                        ...
                                        <th className="px-3 sm:px-4 py-3 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest bg-gray-50/30 hidden sm:table-cell">Center</th>
                                    ...
                                    {paginatedData.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-50/80 transition-colors group">
                                            <td className="px-3 sm:px-4 py-3 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    ...
                                                    <div className="ml-3 sm:ml-4">
                                                        ...
                                                        {/* Mobile-only stats */}
                                                        <div className="flex sm:hidden items-center gap-3 mt-1.5 flex-wrap">
                                                            <div className="flex items-center gap-1">
                                                                <Building2 className="w-2 h-2 text-gray-400" />
                                                                <span className="text-[9px] text-gray-500 truncate max-w-[80px]">{item.center}</span>
                                                            </div>
                                            ...
                                            <td className="px-3 sm:px-4 py-3 whitespace-nowrap hidden sm:table-cell">
                                                <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest bg-gray-100 px-2 py-1 rounded-md">{item.center}</span>
                                            </td>
