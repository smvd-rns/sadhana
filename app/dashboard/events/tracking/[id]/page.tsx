...
import { getEventById, getEventStats, bulkSubmitResponses } from '@/lib/actions/events';
...
export default function EventTrackingPage() {
    ...
    const handleBulkSubmit = async (status: 'coming' | 'not_coming') => {
        if (!event || selectedUserIds.length === 0 || !userData) return;
            ...
            <main className="flex-1 w-full max-w-7xl mx-auto p-4 md:p-8 space-y-6">
                ...
                <div className="bg-white border-2 border-slate-100 rounded-[2rem] shadow-xl shadow-slate-200/50 overflow-hidden flex flex-col">
                    ...
                    {isPM && selectedUserIds.length > 0 && (
                        <div className="m-4 md:m-6 p-4 bg-orange-600 rounded-[1.5rem] text-white flex flex-col md:flex-row items-center justify-between gap-4 animate-in slide-in-from-top-4">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-white/10 rounded-lg">
                                    <CheckCircle2 className="h-5 w-5" />
                                </div>
                                <span className="font-black text-sm">{selectedUserIds.length} users selected for bulk action</span>
                            </div>
                            <div className="flex gap-2 w-full md:w-auto">
                                <button
                                    onClick={() => handleBulkSubmit('coming')}
                                    disabled={isBulkSubmitting}
                                    className="flex-1 md:flex-none px-6 py-2 bg-white text-orange-600 rounded-xl font-black text-xs uppercase tracking-tight hover:shadow-lg transition-all"
                                >
                                    Confirm Coming
                                </button>
                                <button
                                    ...
                                    disabled={isBulkSubmitting}
                                    className="flex-1 md:flex-none px-6 py-2 bg-orange-800 text-white rounded-xl font-black text-xs uppercase tracking-tight hover:bg-orange-900 transition-all"
                                >
                                    Not Coming
                                </button>
                                <button onClick={() => setSelectedUserIds([])} className="p-2 hover:bg-white/10 rounded-xl">
                                    <X className="h-5 w-5" />
                                </button>
                            </div>
                        </div>
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-slate-50/80 border-b border-slate-100">
                                    {isPM && <th className="p-4 md:p-6 w-12">
                                        <input
                                            type="checkbox"
                                            onChange={(e) => {
                                            ...
                                            className="rounded-lg border-2 border-slate-200 text-orange-600 focus:ring-orange-500 h-5 w-5"
                                        />
                                    </th>}
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {filteredReached.length === 0 ? (
                                    ...
                                    return (
                                        <tr key={user.id} className={`group transition-all ${isSelected ? 'bg-orange-50/50' : 'hover:bg-slate-50/30'}`}>
                                            {isPM && <td className="p-4 md:p-6">
                                                <input
                                                    type="checkbox"
                                                    checked={isSelected}
                                                    onChange={() => {
